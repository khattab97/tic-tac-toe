      ...
      for (let idx = 0; idx < arr.length; idx++) {
        ...
        while (j > i) {
          if (arr[j] - arr[i] === arr[i] - arr[idx]) {
            if (
              (arr[j] - arr[i] === 2 &&
                (arr[idx] % 3 !== 0 || arr[i] % 3 !== 2 || arr[j] % 3 !== 1)) ||
              (arr[j] - arr[i] === 1 &&
                (arr[idx] % 3 !== 1 || arr[i] % 3 !== 2 || arr[j] % 3 !== 0))
            ) {
              i++;
              j = arr.length - 1;
            } else {
...
const gameBoard = (() => {
  ...
  const updateGameboard = (player, div, key) => {
    ...
    // Check Draw
    if (gameBoard.boardHistory.length === 9 && modal.style.display === "none")
      populateModal();
  };
...
function check(e) {
  ...
  if (!gameBoard.boardHistory.includes(key)) {
    ...
    if (playerX.classList.contains("player-active"))
      gameBoard.updateGameboard(gameBoard.playerOne, this, key);
